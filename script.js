  ...
  const sendMessage = async (e) => {
    ...
    const fetchEsriSearchResults = async (query) => {
      ...
      try {
        ...
        console.log('Search URL:', searchUrl); // Debug search URL
    ...
    if (userInput.toLowerCase().includes('service url') || userInput.toLowerCase().includes('rest api') || userInput.match(/https?:\/\//)) {
      ...
      try {
        ...
        } else if (userInput.toLowerCase().includes('bia') || userInput.toLowerCase().includes('geospatial') || userInput.toLowerCase().includes('bogs') || userInput.toLowerCase().includes('training')) {
          fallbackText += `Let’s tackle this together! Here’s some info from my BIA knowledge base: ${esriKnowledgeBase.match(/### 24\. BIA Geospatial Training[\s\S]*?(?=###|$)/)[0]} Want to dive deeper?`;
        } else {
