...
It integrates data like maps, satellite imagery, and attributes (e.g., population, temperature) to reveal patterns and trends.
- **Components**:
...
- **Troubleshooting**: Ensure ArcGIS Pro is licensed for required toolboxes (e.g., Spatial Analyst for raster tools).
Verify input data coordinate systems to avoid projection errors.
...
- **Troubleshooting**: Ensure layers share the same audience as the web map.
Check for HTTP/HTTPS mixed content issues.
...
- **Overview**: Highly configurable tool for creating web apps and experiences using flexible layouts, content, and widgets that interact with 2D/3D data.
No-code/low-code for dashboards, apps, etc.
...
3. Add widgets: Drag Map widget and link to a web map; add Chart/Table for data viz;
use Embed for external content like ArcGIS Dashboards.
...
- **Example**: Build a housing dashboard with Map (census tracts), Pie Chart (ownership types), and Table (details).
Link selections to update views.
...
- **Licensing**: ArcGIS Pro/Online requires active licenses;
check portal for concurrent/single-use. If Manage Licensing button missing, verify admin privileges.
- **Cost**: ArcGIS Online is subscription-based;
check credits for usage. No cost details here—refer to Esri pricing.
...
- **Who They Serve**: Indian Affairs (IA) and all federally-recognized Tribes;
regional geospatial coordinators working with Tribes and local BIA agencies; BIA stakeholders for mapping;
represent BIA to other agencies, governments, and the public with authoritative maps.
- **Services**:
...
- **Esri Licenses and Resources**: Access to ArcGIS software through the DOI-BIA ELA as part of software services for Tribes and Indian Affairs;
public datasets accessible via ArcGIS Hub platform.
...
- **Eligibility**: Available to authorized Bureau of Indian Affairs (BIA) employees and employees of federally recognized Tribal Governments.
Limited to approved DOI-BIA ELA program organizations as per the ELA Participation Policy (https://www.bia.gov/sites/default/files/dup/assets/public/pdf/idc015893.pdf).
- **Supported Software**:
...
- **Request Process**: Apply to become approved under DOI-BIA ELA at https://www.bia.gov/service/geospatial-software/apply-ela.
For current versions or support, contact geospatial@bia.gov or the Regional Geospatial Coordinator at MWRGIS@bia.gov.
...
- **Request Process**: Become active ELA participant at https://www.bia.gov/service/geospatial-software/apply-ela. Contact Geospatial Support Help Desk at geospatial@bia.gov or the Regional Geospatial Coordinator at MWRGIS@bia.gov for access.
Registration for events opens ~30 days prior.
...
- **Overview**: The BIA Open Data Portal (https://onemap-bia-geospatial.hub.arcgis.com/) provides national level geospatial data in the public domain to support tribal community resiliency, research, and more.
Maintained by the Branch of Geospatial Support. Serves as a repository for BIA geodata, applications, and resources.
- **Available Data**: Data available for download as CSV, KML, Shapefile;
accessible via web services for application development and data visualization.
Includes datasets like BIA Tracts (interactive map for tracts, parcels, BIA lands).
- **Applications**: StoryMaps, Web Applications, Web Maps for deeper data exploration.
- **Access**: Public access; no cost.
For contributions or support, contact geospatial@bia.gov or the Regional Geospatial Coordinator at MWRGIS@bia.gov.
  ...
  const sendMessage = async (e) => {
    ...
    const fetchServiceMetadata = async (url) => {
      try {
        ...
        let metadata = `**Service Metadata for:** ${data.name ||
          data.documentInfo?.Title || 'Untitled Service'}\n\n`;
    ...
    if (userInput.toLowerCase().includes('service url') || userInput.toLowerCase().includes('rest api') || userInput.match(/https?:\/\//)) {
      ...
      try {
        ...
        const prompt = `
              ...
              or 'let’s tackle this together!', and encourage follow-ups. For basic questions like 'What is GIS?', prioritize the knowledge base.
              For advanced or specific queries, use search results if relevant, then supplement with the knowledge base.
              Include BIA-specific information from the knowledge base for relevant queries (e.g., BOGS contact, software, training).
              Cite sources inline (e.g., Esri Documentation, BIA Website). Learn from the user’s input by adapting responses based on their previous questions if applicable.
              Do not mention AI.
