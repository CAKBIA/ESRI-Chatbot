...
- **Sources**: https://onemap-bia-geospatial.hub.arcgis.com/, https://catalog.data.gov/dataset/bia-bogs-onemap.
  ...
  const sendMessage = async (e) => {
    ...
    // ADDED CODE BLOCK TO ADDRESS BIA GIS TRAINING QUERY
    const lowerCaseInput = userInput.toLowerCase();
    const isBIATrainingQuery = 
      (lowerCaseInput.includes('bia') && lowerCaseInput.includes('training')) ||
      (lowerCaseInput.includes('bogs') && lowerCaseInput.includes('training')) ||
      (lowerCaseInput.includes('bia') && lowerCaseInput.includes('e-learning')) ||
      (lowerCaseInput.includes('bogs') && lowerCaseInput.includes('online courses'));

    if (isBIATrainingQuery) {
      botText = `Let's tackle this together! The BIA Branch of Geospatial Support (BOGS) offers several training programs for BIA and Tribal employees who are active DOI-BIA ELA participants.
      
- **Self-Paced Online Courses**:
  - **Esri E-Learning**: Over 400 resources are available at [https://www.esri.com/training/](https://www.esri.com/training/).
  - **Geospatial Training Services**: Over 40 courses are available at [https://geospatialtraining.com/](https://geospatialtraining.com/).

- **Instructor-Led Online Training**:
  - Events are held by BOGS or USGS.
  - The GEO Project is offered in partnership with Mississippi State University.
  - Esri also provides instructor-led training.

To request access to training, you must be an active DOI-BIA ELA participant. You can start the request process at [https://www.bia.gov/service/geospatial-software/apply-ela](https://www.bia.gov/service/geospatial-software/apply-ela). For more information, you can contact the Geospatial Support Help Desk at geospatial@bia.gov or the Regional Geospatial Coordinator at MWRGIS@bia.gov.

Want to dive deeper? I'm here to help!`;

      setMessages((curr) => [...curr, { text: botText, sender: 'bot' }]);
      setIsLoading(false);
      return; // Exit the function after providing the hardcoded response
    }
