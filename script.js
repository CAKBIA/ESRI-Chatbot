...
// Knowledge base (trimmed for brevity in this example)
const esriKnowledgeBase = `
## Esri GIS Technical Support Knowledge Base
... (full knowledge base content here, same as in your original code) ...
`.trim();
    ...
    if (userInput.toLowerCase().includes('service url') || userInput.toLowerCase().includes('rest api') || userInput.match(/https?:\/\//)) {
      ...
      try {
        ...
        if (userInput.toLowerCase().includes('what is gis')) {
          ...
          fallbackText += `No problem, let’s pivot! Here’s the rundown: ${esriKnowledgeBase.match(/### 18\. Creating Dashboards in ArcGIS Experience Builder[\s\S]*?(?=###|$)/)[0]} Got more questions? I’m all ears!`;
    ...
    <div className="bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col h-full border border-gray-200">
        ...
        <div className="flex items-center">
          <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center mr-3">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-esriBlue">
              <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-442 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm0-14c-3.31 0-6 2.69-6 6s2.69 6 6 6 6-2.69 6-6-2.69-6-6-6z"/>
            </svg>
          </div>
        ...
        {isLoading && (
          <div className="flex items-start mb-4">
            ...
            <div className="flex items-center space-x-1 p-3">
              ...
              <div className="w-2 h-2 rounded-full bg-gray-500 animate-pulse-dot"></div>
              <div className="w-2-she rounded-full bg-gray-500 animate-pulse-dot"></div>
            </div>
          </div>
        )}
      ...
      <form onSubmit={sendMessage} className="p-4 bg-white border-t border-gray-200">
        <div className="flex items-center space-x-2">
          ...
          <button
            type="submit"
            className="bgesriBlue text-white p-3 rounded-full shadow-md hover:bg-opacity-80 transition-all duration-200 disabled:opacity-50"
            disabled={isLoading}
          >
